...
exists for: real two-a-side battles on Gen 2, run by the engine's own
battle sim.

- `lib/doubles2.lua` decorates the engine's battle instance in place the
  moment it exists (the `battle.started` event fires inside `Battle.new`,
  before the battle screen sees the object) and drives four-actor rounds
  through the engine's own primitives: `useMove`, priority, effective
  speed, experience, the per-mon residual ticks.  Damage, accuracy, crits,
  effects, items and weather ARE the engine's code on the same battle
  object the real battle stage renders.
- A named target is honoured per action; a fainted target falls through to
  the side's other slot.  Mid-round faints pay experience and leave the
  slot empty -- the engine's 1v1 replacement never fires mid-round.  Bench
  members walk in at the end of the round, and the moment the battle comes
  back down to 1v1 the engine's own turn loop takes it back, stock.
- Wired behind CRYSTAL 2V2 (BETA) in Mod Settings, off by default: trainer
  2v2 fields the trainer's second able mon beside the lead, and wild
  doubles roll the second foe from the CURRENT map's own table through the
  engine's `World:rollWild` -- never a made-up spawn.  Roamers stay
  strictly 1v1.
- Known limits of the beta: the second foe's HP plate and sprite do not
  render yet (its text, its attacks and your damage against it do), the
  player-side partner and the aim menu are the next chunk, and Battle Art's
  voxel stage gets a second anchor with that same chunk.  27 headless
  checks pin the core.
# Changelog
