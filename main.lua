...
return function(mod)
  ...
  -- ------------------------------------------------------------------
  -- Crystal 2v2: real doubles over the engine's own Gen 2 battle sim.
  -- lib/doubles2.lua decorates the engine's battle instance in place the
  -- moment it exists -- battle.started fires inside Battle.new, before
  -- the battle screen ever sees the object -- and drives four-actor
  -- rounds through the engine's own useMove / priority / speed /
  -- experience machinery.  When the round comes back down to 1v1 the
  -- engine's own turn loop takes the battle back, stock.
  -- ------------------------------------------------------------------
  -- lib loading: mod:read returns the file text, so the module is built
  -- with load() here; a missing file never kills the mod's Gen 1 paths.
  local doubles2Gen2 = nil
  do
    local src2 = mod:read("lib/doubles2.lua")
    if src2 then
      local chunk, err = load(src2, "@double_battles/lib/doubles2.lua")
      if chunk then
        local ok, mod2 = pcall(chunk)
        if ok and type(mod2) == "table" then doubles2Gen2 = mod2 end
      else
        mod.log:warn("doubles2 load failed: %s", tostring(err))
      end
    end
  end
